...
// --------------------------PODCASTS--------------------------
    public function addPodcast(Request $request, Playlist $playlist)
    {
        ...
        if ($playlist->podcasts()->where('podcast_id', $podcastId)->exists()) {
            return response()->json(['message' => 'El podcast ya está en la playlist'], 409);
        }

        $playlist->podcasts()->attach($podcastId);
        return response()->json(['message' => 'Podcast agregado a la playlist exitosamente'], 200);
    }
    ...
    {
        if (!$playlist->podcasts()->where('podcast_id', $podcastId)->exists()) {
            return response()->json(['error' => 'Podcast no encontrado en la playlist'], 404);
        }
